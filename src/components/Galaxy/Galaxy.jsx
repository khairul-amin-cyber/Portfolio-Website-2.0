...
vec3 StarLayer(vec2 uv, float uTime, float uSpeed, float uGlowIntensity, float uTwinkleIntensity) {
  vec3 col = vec3(0.0);
  ...
  for (int y = -1; y <= 1; y++) {
    for (int x = -1; x <= 1; x++) {
      ...
      float flareSize = size * 0.5;

      // Force star color: white or light blue
      vec3 base;
      float seedColor = Hash21(si + 5.0);
      if (seedColor < 0.5) {
        base = vec3(1.0, 1.0, 1.0);       // white
      } else {
        base = vec3(0.6, 0.8, 1.0);       // light blue
      }

      vec2 pad = vec2(tri(seed * 34.0 + uTime * uSpeed / 10.0),
                      tri(seed * 38.0 + uTime * uSpeed / 30.0)) - 0.5;

      float star = Star(gv - offset - pad, flareSize, uGlowIntensity);
...
void main() {
  ...
  for (float i = 0.0; i < 1.0; i += 1.0 / NUM_LAYER) {
    ...
    col += StarLayer(uv * scale + i * 453.32, uTime, uSpeed, uGlowIntensity, uTwinkleIntensity) * fade;
  }
